...
def train_model():
    ...
    results = model.train(
        ...
        epochs=25,  # увеличено количество эпох
        imgsz=640,
        batch=4,  # увеличен размер батча
        device='cpu',  # использование GPU
        patience=10,
        save=True,
        project='runs2/train',
        ...
        optimizer='SGD',  # можно использовать оптимизатор SGD для лучшей производительности
        lr0=0.001,  # начальная скорость обучения
        lrf=0.01,  # конечная скорость обучения
        momentum=0.9,  # параметр импульса
        weight_decay=0.0005  # регуляризация
    )
    
    # Копируем лучшую модель
    best_model = Path('runs2/train/pollution_detector/weights/best.pt')
    if best_model.exists():
        shutil.copy2(best_model, 'baseline2.pt')
        print(f"Лучшая модель сохранена как baseline2.pt")
    else:
